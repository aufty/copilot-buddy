...
{
	private const string DefaultPipeName = "TaskbarBuddy.Presentation.v1";
	...
	public static async Task<int> RunAsync(string[] args)
	{
		...
		try
		{
			...
			if (response is null)
			{
				Console.Error.WriteLine("Taskbar Buddy returned no response.");
				return 1;
			}
			if (!response.Success)
			{
		...
		{
			Console.Error.WriteLine("Taskbar Buddy is not running or did not respond.");
			return 1;
		}
	}
	...
	private static void PrintUsage()
	{
		...
		Console.Error.WriteLine("  buddyctl show \"Build needs your approval\"");
		Console.Error.WriteLine("  buddyctl dismiss");
		Console.Error.WriteLine("  buddyctl visible on|off");
		Console.Error.WriteLine("  buddyctl rainbow-present");
	}
}
