    ...
    private static void Main()
    {
        ...
        if (arguments.Contains("--assistant-broker", StringComparer.OrdinalIgnoreCase))
        {
            ...
            string? cliPath = cliPathIndex >= 0 && cliPathIndex + 1 < arguments.Length
                ...
                : null;
            TaskbarBuddy.Copilot.CopilotSessionBrokerHost.RunAsync(cliPath).GetAwaiter().GetResult();
            return;
        }
        ApplicationConfiguration.Initialize();
        Application.SetUnhandledExceptionMode(UnhandledExceptionMode.ThrowException);
        using Mutex instance = new(true, "Local\\TaskbarBuddy.Composition.v1", out bool firstInstance);
        if (!firstInstance)
        {
            return;
        }
        try
        {
            ...
            string executablePath = Environment.ProcessPath
                ?? throw new InvalidOperationException("Could not locate the Taskbar Buddy executable.");
...
internal sealed partial class ReplayWindow : Form
{
    ...
    private bool directDrag = Environment.GetCommandLineArgs().Contains("--direct-drag");
    ...
    public ReplayWindow(IStoredAssistantSessions sessions, SessionSettings settings)
    {
        ...
        Text = ReplayOnly ? "Taskbar Buddy - Composition replay" : "Taskbar Buddy - Composition drag trial";
    ...
    protected override void OnShown(EventArgs args)
    {
        ...
        if (TaskbarMode)
        {
            trayIcon = new NotifyIcon { Icon = SystemIcons.Application, Text = "Taskbar Buddy", ContextMenuStrip = exitMenu, Visible = true };
            StartPassiveMotion();
    ...
    private void SaveDragTrace()
    {
        ...
        dragTrace = null;
        string directory = Path.Combine(Path.GetTempPath(), "TaskbarBuddy", "drag-traces");
    ...
    private void RunInteractionTestStep()
    {
        ...
        {
            smokeSettled = Vector3.Distance(PositionOf(controller!.Snapshot), dragTarget) <= 1 &&
                dragAnimationSubmissions == 3;
        }
    ...
    protected override void OnFormClosed(FormClosedEventArgs args)
    {
        ...
        if (interactionSmoke && controller is not null)
        {
            ...
            File.WriteAllText(Path.Combine(AppContext.BaseDirectory, "interaction-smoke.json"),
                JsonSerializer.Serialize(new { Passed = passed, SettledWithoutRetarget = smokeSettled, MovingRetargets = nativeMovingRetargets, Regrabbed = smokeRegrabbed, CaptureReleased = smokeCaptureReleased, CompletedReleases = completedNativeReleases, Samples = interactionTestSamples }));
            if (!passed)
            {
                Environment.ExitCode = 1;
            }
