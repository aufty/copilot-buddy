...
internal sealed partial class ReplayWindow
{
    ...
    private void ShowSkillMenu()
    {
        ...
        Size menuSize = new((int)Math.Round(360 * dpiScale), (int)Math.Round(226 * dpiScale));
    ...
    private void ShowSessionError(string message)
    {
        ...
        if (HasSessionAction)
        {
            trayIcon?.ShowBalloonTip(5000, "Taskbar Buddy", message, ToolTipIcon.Warning);
        }
        else
        {
...
internal sealed class SessionSettings
{
    ...
    private static string SettingsPath => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "TaskbarBuddy", "settings.json");
