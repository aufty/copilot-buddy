...
internal sealed class PixelSkillMenu : Form
{
    ...
    public PixelSkillMenu()
    {
        ...
        AccessibleName = "Taskbar Buddy special skills";
    ...
    public void Present(Form owner, Point location, float dpiScale)
    {
        ...
        ClientSize = new Size((int)Math.Round(360 * scale), (int)Math.Round(226 * scale));
        Location = location;
        handoffBounds = new Rectangle((int)(12 * scale), (int)(61 * scale),
            ClientSize.Width - (int)(24 * scale), (int)(74 * scale));
        storeBounds = new Rectangle((int)(12 * scale), (int)(137 * scale),
            ClientSize.Width - (int)(24 * scale), (int)(74 * scale));
    ...
    protected override void OnPaint(PaintEventArgs args)
    {
        ...
        using SolidBrush itemBackground = new(handoffHovered
            ? Color.FromArgb(63, 83, 96)
            : Color.FromArgb(42, 39, 51));
        args.Graphics.FillRectangle(itemBackground, handoffBounds);
        using Pen itemEdge = new(handoffHovered
            ? Color.FromArgb(255, 203, 77)
            : Color.FromArgb(75, 71, 88), Math.Max(1, 2 * scale));
        args.Graphics.DrawRectangle(itemEdge, handoffBounds.X, handoffBounds.Y,
            handoffBounds.Width - 1, handoffBounds.Height - 1);

        Rectangle labelBounds = new(handoffBounds.X + (int)(16 * scale), handoffBounds.Y,
            handoffBounds.Width / 2, handoffBounds.Height);
        TextRenderer.DrawText(args.Graphics, "Handoff", itemFont, labelBounds,
            Color.FromArgb(247, 244, 231), TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPrefix);
        Rectangle shortcutBounds = new(handoffBounds.X + handoffBounds.Width / 2, handoffBounds.Y,
            handoffBounds.Width / 2 - (int)(16 * scale), handoffBounds.Height);
        TextRenderer.DrawText(args.Graphics, "Alt+Shift+H", shortcutFont, shortcutBounds,
            Color.FromArgb(255, 203, 77), TextFormatFlags.Right | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPrefix);
    ...
    protected override void OnMouseMove(MouseEventArgs args)
    {
        ...
        if (handoffHovered != hovered || storeHovered != store)
        {
            handoffHovered = hovered;
            storeHovered = store;
            Cursor = hovered || store ? Cursors.Hand : Cursors.Default;
            Invalidate();
        }
    }
    ...
    protected override void OnMouseLeave(EventArgs args)
    {
        ...
        if (handoffHovered || storeHovered)
        {
            handoffHovered = false;
            storeHovered = false;
    ...
    protected override void OnMouseDown(MouseEventArgs args)
    {
        ...
        if (args.Button == MouseButtons.Left && handoffBounds.Contains(args.Location))
        {
            Hide();
