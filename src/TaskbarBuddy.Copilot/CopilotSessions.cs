...
public sealed class CopilotSessions(string? executablePath = null) : IAssistantSessions
{
    ...
    private static void RecordAttachment(TerminalSession terminal, string stage, Exception? error = null)
    {
        try
        {
            string directory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "TaskbarBuddy", "connections");
        ...
        CancellationToken cancellationToken)
    {
        ...
        string suffix = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(uniqueSource)))[..12]
            .ToLowerInvariant();
        string handoffDirectory = Path.Combine(Path.GetTempPath(), "TaskbarBuddy", "handoffs");
